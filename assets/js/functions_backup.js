...
$('.downNavArrow').on('click',function(e){
  ...
  var defIconShown = true;
  $('.circlePlaceholder').hover(function(){
    
      // On hover, hide the default icon and indicate expansion
      var $defIcon = $(this).children(":nth-child(2)");
      var $expandIcon = $(this).find(".expandIcon");
      
    if (defIconShown){
          
      $defIcon.fadeOut('medium', function(){
        defIconShown = false;
        if (!allowToShow) return;
        $expandIcon.fadeIn('medium', function(){
          // $('#expandIcon-graphic').addClass('expandIcon-enlarge');
        });
        $expandIcon.css('transform', 'scale(1.2,1.2)');

      });
    } else {
      $expandIcon.css('transform', 'scale(1,1)');
      $expandIcon.fadeOut('medium', function(){
        defIconShown = true;
        if (!allowToShow) return;
        $defIcon.fadeIn('medium');
